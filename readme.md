...
6. Графическая визуализация
   ...
   - В качестве графического модуля выступает PyQt. 
...
Таким образом, уже сейчас проект:
...
## To Do
Ниже перечислены основные задачи, которые ещё предстоит реализовать или доработать:

1. Перевод между валютами
    На данный момент все расчёты ведутся без конвертации. Для более реалистичного сценария необходимо:

    - Добавить поддержку несколько типов валют (RUB, USD, EUR).
    - Реализовать автоматическую конвертацию при покупке/продаже активов, если валюта портфеля и валюта актива различаются.


2. Более подробный вывод о доходах по каждому активу
    В настоящее время приложение выводит только ежемесячную прибыль, налог, выкуп/продажу паев и ежемесячный капитал. Для получения полной картины доходности стоит:

    - Детализировать прибыль: показывать, сколько принесли акции, облигации, металлы, депозиты отдельно.

3. Больше графиков и более приятный пользовательский интерфейс

    Пока что приложение содержит только графики капитала, размера портфеля и цен акций в зависимости от месяца. Для улучшения наглядности можно добавить:

        - Динамику доходов и расходов.
        - Доходы по каждому из активов.

4. Валидация покупок: проверять, достаточно ли у фонда капитала для приобретения актива.
5. Провести более масштабное тестирование.
